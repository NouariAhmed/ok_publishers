...
?>
    <div class="container-fluid py-4">
      ...
      if (isset($_SESSION['create_update_success']) && $_SESSION['create_update_success'] === true) {
        ...
        header("Location: display_publisher_languages.php?create_update_success=1");
        exit;
    }
    ...
    if (isset($_SESSION['item_not_found']) && $_SESSION['item_not_found'] === true) {
     ...
     if (!empty($id)) {
         $stmt = mysqli_prepare($conn, "SELECT * FROM publisher_languages WHERE id = ?");
         ...
         if (mysqli_num_rows($result) > 0) {
             ...
             // Redirect to the display_publisher_languages page after item not found
     ...
     if ($_SERVER['REQUEST_METHOD'] === 'POST') {
         if (isset($_POST['updateData'])) {
             ...
             if (empty($publisher_language)) {
                echo "<div class='alert alert-danger text-right text-white'>اللغة مطلوبة</div>";
                 ...
                 $stmt = mysqli_prepare($conn, "UPDATE publisher_languages SET publisher_language = ? WHERE id = ?");
                 mysqli_stmt_bind_param($stmt, "si", $publisher_language, $id);
        ...
        ?>
          <form role="form" action="<?php echo $_SERVER['PHP_SELF'] . '?id=' . $id; ?>" method="post">
              <h4 class="mb-3">تحديث اللغة</h4>
              <div class="form-group">
                  <label class="form-label">اللغة :</label>
                  <input type="text" name="publisher_language" class="form-control border pe-2" value="<?php echo htmlspecialchars($publisher_language); ?>" required>
              </div>
          ...
          <hr>
          <a href="display_publisher_languages.php" class="btn btn-secondary">العودة إلى قائمة اللغات</a>
<?php
