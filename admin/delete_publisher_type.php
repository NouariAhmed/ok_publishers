...
?>
    ...
    <h4 class="mb-3">حذف نوع دار نشر</h4>
      <?php
            ...
            if (isset($_SESSION['delete_success']) && $_SESSION['delete_success'] === true) {
                ...
                header("Location: display_publisher_types.php?delete_success=1");
                exit;
            }
            ...
            if (isset($_GET['id']) && is_numeric($_GET['id'])) {
                ...
                // Check if there are any records in the publishers table referencing the publisher_type
                $stmt = mysqli_prepare($conn, "SELECT id FROM publishers WHERE publisher_type_id = ?");
                ...
                if (mysqli_stmt_num_rows($stmt) === 0) {
                    if (isset($_POST['confirm'])) {
                        // Delete publisher_type 
                        $sql = "DELETE FROM publisher_types WHERE id = ?";
                        ...
                        mysqli_stmt_close($stmt);
                            ...
                            $sql = "SELECT publisher_type FROM publisher_types WHERE id = ?";
                            ...
                            echo '
                            <p class="text-right">هل أنت متأكد أنك تريد حذف نوع دار النشر: ' . htmlspecialchars($typeData['publisher_type']) .' ؟</p>
                        ...
                        <a href="display_publisher_types.php" class="btn btn-secondary">لا</a>
                        ';
                    }
                } else {
                    echo '<div class="alert alert-warning text-right">لا يمكن حذف نوع دار النشر لأنه مستخدم بالفعل من قبل إحدى دور النشر.</div>';
                    echo '<a href="display_publisher_types.php" class="btn btn-secondary">العودة إلى قائمة أنواع دور النشر</a>';
                }
                mysqli_stmt_close($stmt);
